...
- **Song tools are offered only on a typed request.** `AnthropicBrain` builds its tool list per call: song tools when `trigger === "userRequest"`, `compose_song` only with no song active, the editing tools only with one. A tick, an `abletonChanged` or a MIDI note may talk and use the Live tools but must never touch the song plan — that is a product decision, not an optimisation.
